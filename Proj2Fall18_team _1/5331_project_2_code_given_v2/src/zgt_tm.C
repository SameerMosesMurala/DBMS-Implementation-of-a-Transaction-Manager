...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 {
...
#endif
   pthread_t thread1;
   //The transaction information is stored in the paramas structure
   struct param *nodeinfo = (struct param*)malloc(sizeof(struct param));
   nodeinfo->tid = tid;
   nodeinfo->obno = obno;
   nodeinfo->Txtype = ' ';
   ...
   int status;
   //A thread is created and the read method is passed as an arguement to the thread along with node info as params
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
 {
   ...
   nodeinfo->count = --SEQNUM[tid];
   int status;
   //A thread is created and the write method is passed as an arguement to the thread along with node info as params
...
int zgt_tm::CommitTx(long tid, int thrNum)
 {
    ...
    int status;
	//A thread is created and the write method is passed as an arguement to the thread along with node info as params
    status = pthread_create(&threadid[thrNum], NULL ,committx,(void*)nodeinfo);	// fork a thr with tid
    if (status){
      printf("error: return code from pthread_create() is:%d\n", status);
      exit(-1);
    }
...
int zgt_tm::AbortTx(long tid, int thrNum)
 {
    ...
    int status;
	//A thread is created and the write method is passed as an arguement to the thread along with node info as params
    status = pthread_create(&threadid[thrNum], NULL, aborttx,(void*)nodeinfo);	// Fork a thr with tid
    if (status){
      printf("ERROR: return code from pthread_create() is:%d\n", status);
      exit(-1);
    }
